...
- [x] **User upload of new media + auto-analysis** — New page `app/pages/5_Upload_Media.py` under Media Library nav. Multi-file uploader (images + videos, including HEIC). Per file: duplicate check on filename+file_size (real dup → skip with message; filename-only collision → rename with `_YYYYMMDD_HHMMSS` suffix), upload to main Drive folder, analyze via Claude Vision, upsert to media_library with status='analyzed'. Reused code: new `upload_to_main_folder()` in google_drive.py, new `process_image_bytes()`/`process_video_bytes()` in media_indexer.py, new `find_duplicate_by_name_size()`/`find_any_with_filename()` in media_queries.py. DONE 2026-04-21.
- [x] **Scenario expander on Review post cards** — Reel posts in Review page now show an expander with the scenario title, concept description, mood, characters used, caption hook, and the exact motion_prompt sent to the video model. Lets manager trace what was asked vs. what was rendered. `_fetch_scenario_info()` in review.py reads from `generated_scenarios` via `posts.scenario_id`. DONE 2026-04-24.
- [x] **Supabase Auth login gate (Google OAuth + email/password)** — Ported from suppliers project. `src/auth.py` implements `check_auth`, `handle_oauth_callback`, `login_form`, `logout` (PKCE flow, `st.cache_resource`-backed verifier store to survive Streamlit reruns). `src/database.py` now creates client with `ClientOptions(flow_type="pkce")`. `app/main.py` runs `handle_oauth_callback()` + auth gate before nav, wraps login in its own `st.navigation` to suppress page-discovery leak, and adds sidebar logout. `APP_URL` added to `.env` (localhost:8501) + `.streamlit/secrets.toml.example` (Streamlit Cloud URL). Manual prereqs to deploy: set `APP_URL` in Streamlit Cloud secrets + add it to Supabase Auth → URL Configuration → Redirect URLs. Shared Supabase project means users from hotelPandL/suppliers can sign in here. DONE 2026-05-16.
